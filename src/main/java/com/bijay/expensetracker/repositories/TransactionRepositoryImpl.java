...
public class TransactionRepositoryImpl implements TransactionRepository {
    ...
    private static final String SQL_FIND_ALL ="SELECT TRANSACTION_ID, USER_ID, CATEGORY_KEY, AMOUNT, NOTE, "+
            "TRANSACTION_DATE FROM ET_TRANSACTIONS WHERE USER_ID=? AND CATEGORY_KEY=?";

    private static final String SQL_FIND_BY_ID = "SELECT TRANSACTION_ID, USER_ID, CATEGORY_KEY, AMOUNT, NOTE, " +
            "TRANSACTION_DATE FROM ET_TRANSACTIONS WHERE USER_ID=? AND CATEGORY_KEY=? AND TRANSACTION_ID=?";
    ...
    private static final String SQL_UPDATE = "UPDATE ET_TRANSACTIONS SET AMOUNT=?, NOTE=?, TRANSACTION_DATE=? " +
            "WHERE USER_ID=? AND CATEGORY_KEY=? AND TRANSACTION_ID=?";

    private static final String SQL_DELETE = "DELETE FROM ET_TRANSACTIONS WHERE USER_ID=? AND CATEGORY_KEY=? AND " +
            "TRANSACTION_ID=?";
    ...
    public List<Transaction> findAll(Integer userId, Integer categoryKey) {
        return jdbcTemplate.query(SQL_FIND_ALL, new Object[]{userId, categoryKey}, transactionRowMapper);
    }
    ...
    public Transaction findById(Integer userId, Integer categoryKey, Integer transactionId) throws EtResourceNotFoundException {
        try {
            return jdbcTemplate.queryForObject(SQL_FIND_BY_ID, new Object[]{userId, categoryKey, transactionId},
    ...
    public void update(Integer userId, Integer categoryKey, Integer transactionId, Transaction transaction) throws EtBadRequestException {
        try {
            ...
            transaction.getTransactionDate(), userId, categoryKey, transactionId});
        } catch (Exception e) {
            throw new EtBadRequestException("Invalid Request !!");
        }
    }
    ...
    public void removeById(Integer userId, Integer categoryKey, Integer transactionId) throws EtResourceNotFoundException {
        int count = jdbcTemplate.update(SQL_DELETE, new Object[]{userId, categoryKey, transactionId});
        if (count == 0)
            throw new EtResourceNotFoundException("Transaction not Found !!");
    }
